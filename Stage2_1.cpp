...
Stage2_1::Stage2_1()
{
	...
	StageX3 = new HitObject();
	StageX3->mSprite->SetTexture(gpTextureGear05);
	StageX3->SetSize(0.2f, 0.2f);

	StageX4 = new HitObject();
	StageX4->mSprite->SetTexture(gpTextureGear05);
	StageX4->SetSize(0.2f, 0.2f);

	StageX5 = new HitObject();
	StageX5->mSprite->SetTexture(gpTextureGear05);
	StageX5->SetSize(0.2f, 0.2f);
	...
	//マップチップファイル読み込み
	maptip = new MaptipFileRead("assets/Map_CSV/Stage2-1.csv");
...
void Stage2_1::Update()
{
	...
	{	// それ以外なら
		...
		for (int i = 0; i < maptip->mapObj.size(); i++)
		{
			...
			if (maptip->mapObj[i]->mapNo == 9)
			{
				StageX3->SetPosition(maptip->mapObj[i]->mSprite->mCenter.x, maptip->mapObj[i]->mSprite->mCenter.y);
			}
			if (maptip->mapObj[i]->mapNo == 10)
			{
				StageX4->SetPosition(maptip->mapObj[i]->mSprite->mCenter.x, maptip->mapObj[i]->mSprite->mCenter.y);
			}
			if (maptip->mapObj[i]->mapNo == 11)
			{
				StageX5->SetPosition(maptip->mapObj[i]->mSprite->mCenter.x, maptip->mapObj[i]->mSprite->mCenter.y);
			}
		...
		for (int i = 0; i < MAX_GEAR; i++)
		{
			...
			if (StageX3->mHitCircle->IsHit(StageGearX->mGear[i]->mHitCircle) == true)
			{
				if (clickCheck == true)
				{
					XA_Play(SOUND_LABEL_SE010, 0.8f);
					clickCheckAfter = false;
					ModeMoveFloorX3 = true;
					StageGearX->mGear[i]->Activate(false);
					StageX3->Activate(false);
					BaseScene::Stop(5);
				}
			}
			else
			...
			if (StageX4->mHitCircle->IsHit(StageGearX->mGear[i]->mHitCircle) == true)
			{
				if (clickCheck == true)
				{
					XA_Play(SOUND_LABEL_SE010, 0.8f);
					clickCheckAfter = false;
					ModeMoveFloorX4 = true;
					StageGearX->mGear[i]->Activate(false);
					StageX4->Activate(false);
					BaseScene::Stop(5);
				}
			}
			else
			...
			if (StageX5->mHitCircle->IsHit(StageGearX->mGear[i]->mHitCircle) == true)
			{
				if (clickCheck == true)
				{
					XA_Play(SOUND_LABEL_SE010, 0.8f);
					clickCheckAfter = false;
					ModeMoveFloorX5 = true;
					StageGearX->mGear[i]->Activate(false);
					StageX5->Activate(false);
					BaseScene::Stop(5);
				}
			}
			else
		...
		if (LightPowerX)
		{
			...
			StageX3->mSprite->mColor.r = 3.0f;
			StageX4->mSprite->mColor.r = 3.0f;
			StageX5->mSprite->mColor.r = 3.0f;
		}
		else
		{
			StageX->mSprite->mColor.r = 1.0f;
			StageX2->mSprite->mColor.r = 1.0f;
			StageX3->mSprite->mColor.r = 1.0f;
			StageX4->mSprite->mColor.r = 1.0f;
			StageX5->mSprite->mColor.r = 1.0f;
		}
